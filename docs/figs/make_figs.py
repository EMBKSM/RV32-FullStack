...
INK="#1f2a44"; FILL="#eef2f7"; FILL2="#e2e8f0"; ACC="#0d9488"; ACC2="#b45309"
MUT="#64748b"; LINE="#334155"; HL="#fde68a"; WHITE="#ffffff"; GRID="#cbd5e1"
...
ax.add_patch(FancyBboxPatch((6,6),88,55,boxstyle="round,pad=0.2,rounding_size=2.5",
    fc="#fffbeb",ec=ACC2,lw=1.8)); lbl(ax,11,57.5,"Zynq PL  —  rv32_platform  (XC7Z020)",fs=11,c=ACC2,ha="left",bold=True)
...
box(ax,30,22,39,12,"NPU  ·  16x16 INT8 systolic GEMM\n256 MAC (200 DSP48 + 56 LUT-MAC)",fc="#fef3c7",fs=9,bold=True)
...
for i,(s,d) in enumerate(stages):
    ...
    xs.append(x+w/2)
    if i>0: box(ax,x-gap+0.3,47,gap-1,18,"",fc="#cbd5e1",ec=MUT,lw=1,r=0.01)  # pipeline reg
...
box(ax,82,40,16,22,"Requantize\nINT32 -> INT8\nclip(acc*m\n>> sh)",fc="#fef3c7",fs=8.5,bold=True,ec=ACC2)
...
# NPU self compute
ax.add_patch(Rectangle((X[3]-5,46),10,16,fc="#fef3c7",ec=ACC2,lw=1.3,zorder=5))
...
# npu chain
box(ax,62,38,34,8,"npu_top16  ->  npu_array  ->  npu_pe",fc="#fef3c7",fs=8.5,bold=True,ec=ACC2)
...
cols=["#e2e8f0",MUT,"#93c5fd","#60a5fa",ACC,ACC2]
...
ax2.tick_params(colors=MUT); ax2.grid(axis="y",color="#eef2f7",zorder=0)
