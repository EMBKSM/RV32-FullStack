...
module tb_csr_file;
  ...
  task automatic csrop(input logic [11:0] a, input logic [1:0] cmd,
    ...
    csr_addr=a; csr_cmd=cmd; csr_wdata=d; csr_we=we;
    trap_we=0; is_mret=0; @(posedge clk); csr_we=0; @(negedge clk);
  endtask
  function automatic logic [31:0] rd(input logic [11:0] a);
    csr_addr=a; #1; return csr_rdata;
  endfunction
  ...
  initial begin
    ...
    ck(rd(12'h301)==32'h40000100, "misa RO");
    // mtvec CSRRW
    csrop(12'h305, 2'b01, 32'hDEADBEEF, 1); ck(rd(12'h305)==32'hDEADBEEF, "mtvec RW");
    ck(mtvec_o==32'hDEADBEEF, "mtvec exported");
    // mscratch set/clear (RS/RC)
    csrop(12'h340, 2'b01, 32'h0000000F, 1);
    csrop(12'h340, 2'b10, 32'h000000F0, 1); ck(rd(12'h340)==32'h000000FF, "RS set");
    csrop(12'h340, 2'b11, 32'h0000000F, 1); ck(rd(12'h340)==32'h000000F0, "RC clear");
    // csr_we=0 suppresses write
    csrop(12'h340, 2'b01, 32'h12345678, 0); ck(rd(12'h340)==32'h000000F0, "we=0 no write");

    // trap entry: MIE=1 first
    csrop(12'h300, 2'b01, 32'h00000008, 1);          // mstatus.MIE=1 (bit3)
    csr_addr=0; csr_cmd=0; csr_we=0; trap_we=1; is_mret=0;
    trap_mepc=32'h00000100; trap_mcause=32'd2; trap_mtval=32'h00000ABC;
    @(posedge clk); trap_we=0; @(negedge clk);
    ck(rd(12'h341)==32'h00000100, "trap mepc");
    ck(rd(12'h342)==32'd2, "trap mcause");
    ck(rd(12'h343)==32'h00000ABC, "trap mtval");
    ck(mstatus_o[7]==1'b1 && mstatus_o[3]==1'b0 && mstatus_o[12:11]==2'b11,
       "trap mstatus MPIE/MIE/MPP");

    // MRET restore
    trap_we=0; is_mret=1; @(posedge clk); is_mret=0; @(negedge clk);
    ck(mstatus_o[3]==1'b1 && mstatus_o[7]==1'b1 && mstatus_o[12:11]==2'b00,
       "mret mstatus MIE<=MPIE,MPP=0");

    // ---- 3-point boundary value analysis (write-data min/max, set/clear mask edges) ----
    csrop(12'h340, 2'b01, 32'h00000000, 1); ck(rd(12'h340)==32'h00000000, "BVA RW min");
    csrop(12'h340, 2'b01, 32'hFFFFFFFF, 1); ck(rd(12'h340)==32'hFFFFFFFF, "BVA RW max");
    csrop(12'h340, 2'b11, 32'hFFFFFFFF, 1); ck(rd(12'h340)==32'h00000000, "BVA RC all-ones -> 0");
    csrop(12'h340, 2'b10, 32'h00000001, 1); ck(rd(12'h340)==32'h00000001, "BVA RS bit0");
    csrop(12'h340, 2'b10, 32'h80000000, 1); ck(rd(12'h340)==32'h80000001, "BVA RS bit31");
    ...
    csr_addr=12'h300; csr_cmd=2'b01; csr_wdata=32'hFFFFFFFF; csr_we=1;
    trap_we=1; trap_mepc=32'h00000200; trap_mcause=32'd7; trap_mtval=32'h00000111; is_mret=1;
    @(posedge clk); csr_we=0; trap_we=0; is_mret=0; @(negedge clk);
    ck(rd(12'h341)==32'h00000200 && rd(12'h342)==32'd7, "TIMING trap_we beats csr_we/mret at edge");
